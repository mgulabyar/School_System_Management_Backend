...
// 2. Get All Sections (For dropdowns & selections) [1]
...
// 5. Create a New Subject linked to a Class
exports.createSubject = async (req, res) => {
    try {
        const { name, code, classId } = req.body;
...
// 6. Get All Subjects for a Specific Class
exports.getSubjectsByClass = async (req, res) => {
    try {
        const { classId } = req.params;
        const subjects = await Subject.find({ class: classId }).populate('class');
...
// 8. Get Timetable for a specific Class & Section
exports.getTimetable = async (req, res) => {
    try {
        const { classId, sectionId } = req.params;
        const schedule = await Timetable.find({ class: classId, section: sectionId })
            .populate('subject', 'name code')
            .populate({
                path: 'teacher',
                populate: { path: 'user', select: 'name' }
...
exports.createCalendarEvent = async (req, res) => {
    try {
        ...
        const event = await CalendarEvent.create({ title, description, startDate, endDate, type });
        res.status(201).json({ success: true, message: 'Calendar event/holiday created successfully!', data: event });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
};

// 10. Get Academic Calendar
exports.getCalendarEvents = async (req, res) => {
    try {
        const events = await CalendarEvent.find().sort({ startDate: 1 });
        res.status(200).json({ success: true, count: events.length, data: events });
