...
exports.returnBook = async (req, res) => {
    try {
        ...
        res.status(200).json({
            success: true,
            message: fine > 0 ? `Book returned late! Fine of Rs. ${fine} generated.` : 'Book returned successfully!',
            data: record
        });
...
exports.updateBook = async (req, res) => {
    try {
        ...
        book.rackNo = rackNo || book.rackNo;
    ...
    try {
        ...
        if (book.available < book.quantity) {
            return res.status(400).json({
                success: false,
                message: 'Cannot delete book! Some copies of this book are currently issued out.'
            });
        }
        ...
        res.status(200).json({ 
            success: true, 
            message: 'Book successfully deleted from Library inventory!' 
        });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
