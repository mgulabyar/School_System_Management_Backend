...
exports.addVehicle = async (req, res) => {
  try {
    const { vehicleNo, registrationNo, driverName, driverPhone, capacity } =
      req.body;
...
exports.createRoute = async (req, res) => {
  try {
    ...
    if (routeExists) {
      return res
        .status(400)
        .json({ success: false, message: "Route name already exists!" });
    }
...
exports.allocateTransport = async (req, res) => {
  try {
    ...
    const alreadyAllocated = await TransportAllocation.findOne({
      student: studentId,
    });
    if (alreadyAllocated) {
      return res
        ...
        .json({
          success: false,
          message: "This student is already allocated to a transport route!",
        });
    }
  ...
  try {
    ...
    if (!deleted) {
      return res
        ...
        .json({
          success: false,
          message: "Transport allocation record not found!",
        });
    }
